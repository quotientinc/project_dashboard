...
Provides a unified view of a project with tabs for Details, Edit, Timeline, and Analytics.
...
# Resolve project ID: query params first, session state fallback
project_id = None

if "project_id" in st.query_params:
    project_id = st.query_params["project_id"]

if project_id is None and 'selected_project_id' in st.session_state:
...
if st.query_params.get("project_id") != str(project_id):
...
with col3:
    ...
    if selected_name != project['name']:
        ...
        st.query_params.from_dict({"project_id": str(new_project['id'])})
        st.rerun()
...
valid_tabs = ["Details", "Edit", "Timeline", "Analytics"]
...
# Create tabs
tab_details, tab_edit, tab_timeline, tab_analytics = st.tabs(
    ["Details", "Edit", "Timeline", "Analytics"],
    default=default_tab
)

with tab_details:
    try:
        render_project_details_tab(db, processor, project_id=project_id, default_subtab=default_subtab)
