...
class DataProcessor:
        ...
        if not allocations_df.empty and not time_entries_df.empty:
            ...
            if not project_time.empty:
                ...
                if 'amount' in project_time.columns and project_time['amount'].notna().any():
                    # Calculate cost: use amount when available, fallback to hours x bill_rate
    ...
    def build_hours_sheet_data(
        ...
        for emp in unique_employees:
            ...
            for month_date in months:
                ...
                # Get holidays from months table via db parameter
                holidays_in_month = 0
                if db is not None:
                    try:
    ...
    def build_hours_by_month_data(
        ...
        # No session state available in backend context
        fte_edits = {}
        ...
        for _, emp_data in hours_df.iterrows():
            ...
            # Calculate target hours: FTE x 160 hours/month x number of months
    ...
    def get_performance_metrics(
        ...
        logger.debug(f"Building actuals data from {start_date} to {end_date}")
        ...
        logger.debug(f"Building projected data from {start_date} to {end_date}")
        ...
        logger.debug(f"Building possible data from {start_date} to {end_date}")
    ...
    def _build_actuals_data(
        ...
        # Calculate revenue: Use amount if available, otherwise calculate from hours x bill_rate
        # Priority: 1) time_entries.amount, 2) hours x time_entries.bill_rate
        def calculate_revenue(row):
            if pd.notna(row['amount']) and row['amount'] != 0:
                return row['amount']
            else:
                # Use bill_rate from time_entries (not allocations)
                bill_rate = row.get('bill_rate', 0)
                return row['hours'] * (bill_rate if pd.notna(bill_rate) else 0)
    ...
    def _build_possible_data(
        ...
        # Formula: (working_days - holidays) x (target_allocation - overhead_allocation) x 8 x proration_factor
        cross_join['hours'] = (
    ...
    def combine_actual_projected_smartly(
        ...
        for month_name in all_months:
            ...
            try:
                ...
                logger.exception("Could not parse month name: %s, skipping", month_name)
                continue
            ...
            for entity_id in all_entities:
                ...
                if month_type == 'past':
                    ...
                    if is_active and expected_working_days > 0:
                        ...
                        # Formula: actual + (remaining_days_ratio x projected)
...
def df_to_records(df: pd.DataFrame) -> list[dict]:
    """
    Convert a pandas DataFrame to a list of dicts suitable for JSON responses.

    Handles all common pandas/numpy types that are not natively JSON-serialisable:
      - pd.NaT / np.nan / None -> None
      - pd.Period -> str
