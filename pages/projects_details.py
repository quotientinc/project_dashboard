"""
Project Details tab - displays detailed information about a selected project across 5 subtabs.
"""
...
def render_project_details_tab(db, processor, project_id=None, default_subtab=None):
    """Render the Project Details tab with 5 subtabs: Performance, Team, Timeline, Expenses, Burn Rate.
    ...
    if project_id is not None:
        ...
        selected_project = st.selectbox(
            ...
            index=5 if len(projects_df) > 5 else 0,
        )
    ...
    if not allocations_df.empty and 'allocation_date' in allocations_df.columns:
        # Only check coverage if project has dates defined
    ...
    valid_subtabs = ["Performance", "Team", "Timeline", "Expenses", "Allocation Planner"]
    ...
    with detail_tab3:
        ...
        # Progress
        if project['status'] == 'Active' and pd.notna(project['start_date']) and pd.notna(project['end_date']):
            ...
            st.info("Project dates not defined - cannot show timeline progress")
