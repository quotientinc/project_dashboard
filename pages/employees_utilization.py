...
def _get_working_days_in_range(emp_start_date, emp_end_date, months_df, year, month):
    ...
    This is a module-level helper extracted so it can be shared by both
    _render_monthly_detail_tab and render_combined_utilization_view.
...
def _calculate_monthly_utilization_data(db, processor, selected_year, selected_month,
    ...
    This is a shared helper that extracts the data-calculation logic from
    _render_monthly_detail_tab so it can be reused by the combined view.
...
def _calculate_period_utilization_data(db, processor, start_date, end_date,
    ...
    agg_df['ytd_utilization_pct'] = (
        agg_df['ytd_actual_billable_hours'] / agg_df['ytd_possible_hours'] * 100
    ).fillna(0)
    agg_df['ytd_utilization_pct'] = agg_df['ytd_utilization_pct'].where(
        agg_df['ytd_possible_hours'] > 0, 0
    )
...
**Gov FY convention:** FY2026 = Oct 1, 2025 → Sep 30, 2026""")
    ...
    try:
        ...
        # ==============================
        # Compute period utilization data (needed by both summary and detail)
        # ==============================
